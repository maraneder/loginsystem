...
class LogIn : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        etEmail = findViewById(R.id.log_in)
...
//                        updateUI(null)
    ...
    private fun blockUser(emailAdres: String){

        auth = Firebase.auth
        database = Firebase.database.reference

        val key = emailAdres.replace('.', '-').replace('@', '-')
        val block = Blocked(LocalDateTime.now())

        database.child("Blocked").child(key).setValue(block)
